...
export const createProject = async ({
  ...
  if (!PUTER_WORKER_URL) {
    console.warn("Puter worker URL not configured, cannot create project");
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Failed to save project, response not ok", response);
      return null;
    }
...
export const getProjects = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Failed to fetch projects, response not ok", response);
      return [];
    }
